...
void main() {
  ...
  ANSEL = 0x01; // AN1 pini analog olarak ayarlanýyor.
  ANSEL = 0x02; // AN2 pini analog olarak ayarlanýyor.
  ANSEL = 0x03; // AN3 pini analog olarak ayarlanýyor.
...
if(PORTA.B3==1 && PORTA.B5 == 1)
{
      ...
      Lcd_Cmd(_LCD_CURSOR_OFF);
      LCD_CMD(_LCD_CLEAR) ;
      koordinant();    //KOORDÝNANT FONKSÝYONU ÇAÐIRDIK
      delay_ms(300);
      Lcd_Cmd(_LCD_CURSOR_OFF);
      LCD_CMD(_LCD_CLEAR);
      ...
      Lcd_out(2,1,"TiTRESiM oluyor");
         ...
         servo();          //SERVO FONKSÝYONU ÇAÐIRDIK
      T=ADC_Read(3);
      Tdeger=(6);
